...
\begin{abstract}
Real-time routing in Intelligent Transportation Systems (ITS) is severely bottlenecked by the $O(V^3)$ or $O(m \cdot (n + m \log n))$ computational complexity associated with constructing exact geometric $t$-spanners on large-scale urban topologies. While machine learning has emerged as a promising paradigm to bypass these computational limits, standard neural predictors lack formal topological guarantees, occasionally pruning critical highway segments and bridges, which leads to disconnected subgraphs or unacceptable routing detours. This paper introduces a robust hybrid neural-algorithmic framework that shifts the heavy computational burden to an offline representation learning phase while enforcing strict structural constraints online. By integrating a scale-invariant Graph Neural Network (GNN), Monte Carlo (MC) Dropout for epistemic uncertainty estimation, and an active geometric feedback loop, our model constructs highly sparsified directed spanners while strictly maintaining 100\% Strong Connectivity (Strongly Connected Components - SCC) and the theoretical $t$-stretch factor. Experimental evaluations across four heterogeneous urban fabrics (Manhattan, Eindhoven, Paris, and Rome) demonstrate up to a 279.2x speedup in spanner construction over classic state-of-the-art baselines. Under extreme network corruption, the uncertainty-aware feedback loop successfully maintains the worst-case directed stretch under 1.5, where standard deterministic models exhibit topological collapse.
\end{abstract}
